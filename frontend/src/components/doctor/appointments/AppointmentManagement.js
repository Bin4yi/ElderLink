...
import { doctorAppointmentService } from '../../../services/doctorAppointment'; // ✅ FIXED: Added one more '../'
import toast from 'react-hot-toast';

const AppointmentManagement = () => { // ✅ FIXED: Changed component name to match file name
  ...
  const handleReviewAppointment = async (appointmentId, action) => {
    try {
      console.log('🔄 Starting review process:', { appointmentId, action });
      ...
      <div className="space-y-4">
        {appointments.length === 0 ? (
          ...
          appointments.map(appointment => (
            <div key={appointment.id} className="bg-white p-6 rounded-lg shadow border hover:shadow-md transition-shadow">
...
export default AppointmentManagement; // ✅ FIXED: Export with correct name